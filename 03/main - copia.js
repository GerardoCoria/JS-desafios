...
for (let i = 1; i <= 10 ; i++) 
{
	...
	let nombreMedico = parseInt(prompt(`Ingrese el especialista con el cual desdea realizar la consulta:
		1 para pediatría, 2 para oftalmología, 3 para neurología y 4 para traumatología`));

	while(true)
	{
		if (nombreMedico > 0 && nombreMedico < 6)
		{
			switch (nombreMedico)
			{
				case 1:
		...
		else
		{
			...
			nombreMedico = parseInt(prompt(`Ingrese el especialista con el cual desdea realizar la consulta:
			1 para pediatría, 2 para oftalmología, 3 para neurología y 4 para traumatología`));
		}
